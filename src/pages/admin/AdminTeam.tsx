...
import { fetchAllTeam, createTeamMember, updateTeamMember, deleteTeamMember } from '@/lib/services';
...
export default function AdminTeam() {
  ...
  const load = () => fetchAllTeam().then((d) => setItems(d)).catch(() => {});
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    const payload = { name: form.name, role: form.role, bio: form.bio || null, image: form.image || null, sort_order: form.sort_order, published: form.published };
    try {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((m) => (
          <div key={m.id} className="bg-white border border-stone-100 rounded-xl p-5 text-center">
            ...
            <div className="mt-3 flex items-center justify-center gap-1"><button onClick={() => openEdit(m)} className="p-2 text-stone-500 hover:text-gold"><Pencil className="w-4 h-4" /></button><button onClick={() => remove(m)} className="p-2 text-stone-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div>
          </div>
        ))}
